...
func _ready() -> void:
	print("ChunkManager: _ready() executed. Setting up chunk management timer.")
...
func _process_chunk_queues(desired_chunks: Array[String]) -> void:
	...
	for loading_id: String in loading_chunks:
		var status: ResourceLoader.ThreadLoadStatus = ResourceLoader.load_threaded_get_status(chunk_registry[loading_id])
