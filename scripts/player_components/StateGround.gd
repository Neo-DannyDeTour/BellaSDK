...
func _calculate_target_speed(delta: float, input_dir: Vector2) -> void:
	...
	if Input.is_action_pressed("crouch"):
		...
		player.head.position.y = lerpf(player.head.position.y, player.vault_controller.crouching_depth, delta * 15.0)
	...
	player.sprint_active = Input.is_action_pressed("sprint") and not player.crouching and is_moving and player.can_sprint
...
func _update_components(delta: float, input_dir: Vector2) -> void:
	...
	player.physics_pusher.process_pushes(
		player.interaction_scanner.held_object, 
		player.last_velocity, 
		player.sprinting_speed
	)
