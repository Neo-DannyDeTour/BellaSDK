...
func enter(msg: Dictionary = {}) -> void:
	...
	if can_swing:
		...
		current_rope.apply_impulse(entry_momentum * 1.5, player.global_position - current_rope.global_position)
	...
	var face_pos := Vector3(current_rope.global_position.x, player.global_position.y, current_rope.global_position.z)
	if player.global_position.distance_to(face_pos) > 0.1:
		...
		(tween.tween_property(player, "quaternion", target_transform.basis.get_rotation_quaternion(), 0.3)
...
func exit() -> void:
	...
	var release_forward: Vector3 = Vector3(-player.global_transform.basis.z.x, 0.0, -player.global_transform.basis.z.z).normalized()
	...
	(release_tween.tween_property(player, "quaternion", target_basis.get_rotation_quaternion(), 0.3)
		.set_trans(Tween.TRANS_SINE)
		.set_ease(Tween.EASE_OUT))
		
	(release_tween.tween_property(player.eyes, "rotation", Vector3.ZERO, 0.3)
		.set_trans(Tween.TRANS_SINE)
		.set_ease(Tween.EASE_OUT))
...
func _handle_climbing_and_swinging(delta: float, input_dir: Vector2) -> void:
	...
	if is_climbing_actively and actually_moved:
		...
		player.camera.transform.origin = player.camera.transform.origin.lerp(Vector3.ZERO, delta * 10.0)
