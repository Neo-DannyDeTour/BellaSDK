...
func _load_audio_settings() -> void:
	...
	if visual_cues_toggle:
		var visual_cues: bool = (
			GlobalSettings.get_setting("Accessibility", "visual_audio_cues", false) as bool
		)
		visual_cues_toggle.button_pressed = visual_cues

	if nav_assist_toggle:
		var nav_assist: bool = (
			GlobalSettings.get_setting("Accessibility", "navigation_assist", false) as bool
		)
...
func _set_bus_volume(bus_name: String, slider_value: float) -> void:
	...
	if bus_idx >= 0:
		...
		print(
			"Audio: Volume for '", bus_name, "' set to ", slider_value, " (Muted: ", is_muted, ")"
		)
