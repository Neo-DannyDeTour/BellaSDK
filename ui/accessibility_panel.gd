...
func _load_accessibility_settings() -> void:
	...
	brightness_slider.value = GlobalSettings.get_setting("Settings", "brightness", DEFAULT_BRIGHTNESS)
	contrast_slider.value = GlobalSettings.get_setting("Settings", "contrast", DEFAULT_CONTRAST)
	saturation_slider.value = GlobalSettings.get_setting("Settings", "saturation", DEFAULT_SATURATION)
	...
	sprint_fov_checkbox.button_pressed = GlobalSettings.get_setting("Settings", "disable_sprint_fov", DEFAULT_DISABLE_SPRINT_FOV)
	_apply_fov_settings()
	
	var saved_sens: float = GlobalSettings.get_setting("Settings", "mouse_sensitivity", DEFAULT_SENSITIVITY)
