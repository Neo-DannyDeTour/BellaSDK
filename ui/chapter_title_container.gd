...
## triggered by the [signal Events.chapter_triggered] global event signal.
...
## The internal subviewport rendering the text overlay.
...
func _ready() -> void:
	print("ChapterDisplay: _ready() called. Configuring initial viewport gating.")
	chapter_label.fit_content = false
	chapter_label.clip_contents = false
	chapter_label.modulate.a = 0.0
	chapter_label.visible = false

	# Gate off rendering passes completely until a chapter title is displayed
...
func _on_resized() -> void:
	...
	# Root MarginContainer stretches to full screen
	set_anchors_preset(Control.PRESET_FULL_RECT)

	# SubViewportContainer matches parent
	effect_container.set_anchors_preset(Control.PRESET_FULL_RECT)
	effect_container.custom_minimum_size = Vector2.ZERO

	# Center the label vertically with an offset, spanning full width for [center] BBCode
	var screen_size: Vector2 = get_viewport_rect().size
	var vertical_offset: float = 60.0
	var label_height: float = 120.0

	# Clear anchors so top-left positioning is respected inside the SubViewport
	...
	chapter_label.set_deferred(
		"position", Vector2(0.0, (screen_size.y * 0.5) - (label_height * 0.5) + vertical_offset)
	)
	chapter_label.set_deferred("pivot_offset", Vector2(screen_size.x * 0.5, label_height * 0.5))
...
func _on_chapter_triggered(
	...
	# Enable SubViewport render pipeline for the duration of the animation
	if is_instance_valid(_sub_viewport):
	...
	match anim_style:
		...
		Events.ChapterAnimStyle.GLITCH:
			...
			_play_shader_effect(
				chapter_name, display_duration, preload("res://vfx/chapter_text_glitch.gdshader")
			)
		Events.ChapterAnimStyle.REVEAL:
			print("ChapterDisplay: Triggering REVEAL shader.")
			_play_shader_effect(
				chapter_name, display_duration, preload("res://vfx/chapter_text_reveal.gdshader")
			)
		Events.ChapterAnimStyle.CHROMATIC:
			print("ChapterDisplay: Triggering CHROMATIC shader animation.")
			_play_shader_effect(
				chapter_name, display_duration, preload("res://vfx/chapter_text_chromatic.gdshader")
			)
		...
		Events.ChapterAnimStyle.DISSOLVE:
			...
			_play_shader_effect(
				chapter_name, display_duration, preload("res://vfx/chapter_text_dissolve.gdshader")
			)
		Events.ChapterAnimStyle.LIQUID:
			print("ChapterDisplay: Triggering LIQUID shader.")
			_play_shader_effect(
				chapter_name, display_duration, preload("res://vfx/chapter_text_liquid.gdshader")
			)
		Events.ChapterAnimStyle.HOLOGRAM:
			print("ChapterDisplay: Triggering HOLOGRAM shader.")
			_play_shader_effect(
				chapter_name, display_duration, preload("res://vfx/chapter_text_hologram.gdshader")
			)
		...
		Events.ChapterAnimStyle.NEON:
			...
			_play_shader_effect(
				chapter_name, display_duration, preload("res://vfx/chapter_text_neon.gdshader")
			)
		Events.ChapterAnimStyle.SHATTER:
			print("ChapterDisplay: Triggering SHATTER shader.")
			_play_shader_effect(
				chapter_name, display_duration, preload("res://vfx/chapter_text_shatter.gdshader")
			)
		Events.ChapterAnimStyle.BLUR:
			print("ChapterDisplay: Triggering BLUR shader.")
			_play_blur(
				chapter_name, display_duration, preload("res://vfx/chapter_text_blur.gdshader")
			)
		Events.ChapterAnimStyle.DOOM_MELT:
			print("ChapterDisplay: Triggering DOOM MELT shader.")
			_play_shader_effect(
				chapter_name, display_duration, preload("res://vfx/chapter_text_doom_melt.gdshader")
			)
		...
		Events.ChapterAnimStyle.VHS:
			...
			_play_shader_effect(
				chapter_name, display_duration, preload("res://vfx/chapter_text_vhs.gdshader")
			)
	...
	# Append global deactivation callback to shut down the SubViewport when complete
...
func _play_slam(chapter_name: String, duration: float) -> void:
	...
	# Phase 4: Slam hard down and fade out
	_chapter_tween.set_parallel(true)
