...
func trigger_surface_wipe() -> void:
	...
	water_clear_tween.tween_property(water_mat, "shader_parameter/clear_progress", 0.65, 0.1).set_trans(Tween.TRANS_SINE)
	water_clear_tween.tween_interval(0.1)

	# Phase 3: Finish sweep
	water_clear_tween.tween_property(water_mat, "shader_parameter/clear_progress", 1.2, 0.2).set_trans(Tween.TRANS_CUBIC).set_ease(Tween.EASE_OUT)

	# Phase 4: Fade droplets
	water_clear_tween.tween_property(water_mat, "shader_parameter/drop_intensity", 0.0, 1.0).set_trans(Tween.TRANS_SINE)
	water_clear_tween.parallel().tween_property(water_mat, "shader_parameter/wash_intensity", 0.0, 1.0).set_trans(Tween.TRANS_SINE)
...
func exit_waterfall() -> void:
	...
	waterfall_clear_tween.tween_property(waterfall_mat, "shader_parameter/clear_progress", 1.2, 0.4).set_trans(Tween.TRANS_CUBIC).set_ease(Tween.EASE_OUT)
	waterfall_clear_tween.parallel().tween_property(waterfall_mat, "shader_parameter/wash_intensity", 0.0, 0.4).set_trans(Tween.TRANS_SINE)
	waterfall_clear_tween.parallel().tween_property(waterfall_mat, "shader_parameter/drop_intensity", 0.0, 1.2).set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_IN)
