...
func _apply_visual_tilt(input_dir: Vector2, delta: float) -> void:
	...
	tilt_pivot.rotation.x = lerpf(
		tilt_pivot.rotation.x, target_pitch, tilt_speed * delta
	)
	tilt_pivot.rotation.z = lerpf(
		tilt_pivot.rotation.z, target_roll, tilt_speed * delta
	)
