...
func interact_with(character: CharacterBody3D) -> void:
	print("Interact_Component: Passing interaction to parent from ", character.name)
	interacted.emit(character)


func hover_cursor(character: CharacterBody3D, hit_position: Vector3) -> void:
	print("Interact_Component: hover_cursor called by ", character.name)
	...
	if not is_currently_focused:
		...
		print("Interact_Component: Focus gained.")
		focused.emit()
...
func _process(_delta: float) -> void:
	...
	if current_time - _last_hover_time_msec > 50:
		...
		print("Interact_Component: Focus lost due to timeout.")
		unfocused.emit()
