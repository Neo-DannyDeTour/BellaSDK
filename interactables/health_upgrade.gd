...
func _on_focused() -> void:
	print("HealthUpgrade: _on_focused() - Player focused. Starting beat animation and showing label.")
...
func _on_unfocused() -> void:
	print("HealthUpgrade: _on_unfocused() - Player unfocused. Stopping beat animation and hiding label.")
...
func _on_interacted(character: CharacterBody3D) -> void:
	...
	if is_instance_valid(health_comp) and health_comp.has_method("increase_max_health"):
		...
		print("HealthUpgrade: _on_interacted() - ERROR: No HealthComponent found on ", character.name)
