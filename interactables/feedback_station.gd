...
func _ready() -> void:
	...
	if is_instance_valid(interact_comp):
		# Connect to the signals emitted by your custom Interact_Component
		...
		print("FeedbackStation: ERROR - Interact_Component is missing or not assigned!")
...
func _on_focused() -> void:
	...
	if is_instance_valid(sprite):
		...
		_hover_tween.tween_property(
			sprite, "position:y", _sprite_initial_y + 0.15, 0.4
		).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_IN_OUT)
		
		# Bounce down
		_hover_tween.tween_property(
			sprite, "position:y", _sprite_initial_y, 0.4
		).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_IN_OUT)
...
func _on_unfocused() -> void:
	...
	if is_instance_valid(sprite):
		...
		_hover_tween = create_tween()
		_hover_tween.tween_property(
			sprite, "position:y", _sprite_initial_y, 0.2
