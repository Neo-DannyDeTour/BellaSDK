		...
		for z in range(z_steps + 1):
			...
			if only_spawn_on_floors:
				...
				_process_posture_and_walls(
					hit_result.position, final_pos, standing_xforms, 
					crouching_xforms, climb_xforms, jump_climb_xforms
				)
			else:
...
func _process_posture_and_walls(
	global_floor: Vector3, local_floor: Vector3,
	standing_xforms: Array[Transform3D], crouching_xforms: Array[Transform3D],
	climb_xforms: Array[Transform3D], jump_climb_xforms: Array[Transform3D]
) -> void:
	...
	for dir in dirs:
		...
		if not wall_hit.is_empty():
			...
			if abs(wall_normal.y) < 0.2:
				...
				var h_start: Vector3 = wall_hit.position + (wall_normal * 0.1) + Vector3(0.0, max_jump, 0.0)
				var h_end: Vector3 = wall_hit.position + (wall_normal * -0.5) + Vector3(0.0, max_jump, 0.0)
				...
				if not ledge_hit.is_empty():
					...
					var jump_w_xform: Transform3D = Transform3D().translated(w_pos + (wall_normal * 0.2) + Vector3(0.0, max_jump, 0.0))
...
func _build_sprite_multimesh(xforms: Array[Transform3D], tex: Texture2D, sprite_height: float) -> void:
	if not tex or xforms.is_empty():
		return
		
	print("Building Sprite MultiMesh for ", xforms.size(), " instances at height ", sprite_height, "...")
