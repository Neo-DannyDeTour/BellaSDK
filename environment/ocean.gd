...
const WATER_MESH_HIGH8_K: Mesh = preload(
    "res://assets/ocean_waves/ocean/clipmap_high_8k.obj"
)
const WATER_MESH_HIGH: Mesh = preload(
    "res://assets/ocean_waves/ocean/clipmap_high.obj"
)
const WATER_MESH_LOW: Mesh = preload(
    "res://assets/ocean_waves/ocean/clipmap_low.obj"
)
...
@export_color_no_alpha var water_color: Color = Color(0.01, 0.02, 0.03):
	set(value):
		...
		RenderingServer.global_shader_parameter_set(
			&"water_color", water_color.srgb_to_linear()
		)
...
@export_color_no_alpha var foam_color: Color = Color(0.9, 0.9, 0.95):
	set(value):
		foam_color = value
		RenderingServer.global_shader_parameter_set(
			&"foam_color", foam_color.srgb_to_linear()
		)
	...
	set(value):
		...
		if WATER_MAT:
			WATER_MAT.set_shader_parameter(
				"crest_glow_intensity", crest_glow_intensity
			)
...
@export_range(0.0, 2.0) var aerated_foam_glow: float = 0.5:
	set(value):
		...
		if WATER_MAT:
			WATER_MAT.set_shader_parameter(
				"aerated_foam_glow", aerated_foam_glow
			)
...
@export var parameters: Array[WaveCascadeParameters]:
	set(value):
		...
		for i: int in range(new_size):
			...
			if not value[i].is_connected(
				&"scale_changed", _update_scales_uniform
			):
...
func _process(delta: float) -> void:
	if is_instance_valid(spray_particles):
		...
		var max_dist: float = maxf(
			0.0, (ocean_plane_size / 2.0) - spray_spawn_radius
		)
	...
	if cam:
		var distance_moved: float = _last_cam_pos.distance_to(
			cam.global_position
		)
...
func _setup_wave_generator() -> void:
	...
	displacement_maps.texture_rd_rid = wave_generator.descriptors[
		&"displacement_map"
	].rid
	normal_maps.texture_rd_rid = wave_generator.descriptors[&"normal_map"].rid
	
	RenderingServer.global_shader_parameter_set(
		&"num_cascades", parameters.size()
	)
	RenderingServer.global_shader_parameter_set(
		&"displacements", displacement_maps
	)
...
func _manage_cpu_displacement_textures_updates(delta: float) -> void:
	...
	var time_per_tex: float = (
		_displacement_textures_total_update_interval / float(cache_size)
	)
	...
	if _displacement_textures_update_time > time_per_tex:
		...
		var target_idx: int = cpu_displacement_textures.keys()[
			_texture_loading_index
		]
		_is_reading_back = true
		RenderingServer.call_on_render_thread(
			_do_texture_readback.bind(target_idx)
		)
...
func _do_texture_readback(idx: int) -> void:
	...
	var rid_downsampled: RID = wave_generator.descriptors[
		&"downsampled_map"
	].rid
	var callable: Callable = _on_texture_data_received.bind(idx)
	var err: int = device.texture_get_data_async(
		rid_downsampled, idx, callable
	)
	if err != OK:
		_is_reading_back = false
...
func _setup_cpu_displacement_textures() -> void:
	...
	RenderingServer.call_on_render_thread(
		_do_initial_texture_readback.bind(used_idx)
	)
...
func _do_initial_texture_readback(used_indices: Array[int]) -> void:
	...
	if not wave_generator or not wave_generator.descriptors.has(
		&"displacement_map"
	):
		return
...
func bake_waves_to_res_routine() -> void:
	...
	for frame: int in range(frames_to_bake):
		...
		RenderingServer.force_sync()
		var rid_disp_map: RID = wave_generator.descriptors[
			&"displacement_map"
		].rid
		
		var tex: PackedByteArray = device.texture_get_data(
			rid_disp_map, cascade_to_bake
		)
		var img: Image = Image.create_from_data(
			wave_generator.map_size,
			wave_generator.map_size,
