...
## Uniform set binding the mask storage image. Manages lifecycle state for compute pipelines.
...
## Recursively searches node [param scene_root] to locate a [SunshineCloudsDriverGD].
...
func clear_compute() -> void:
	print(
		"CloudsEditorController: clear_compute() called. Despawning GPU objects to prevent memory leak."
	)
	if rd:
...
## Dispatches compute shader to stamp brush strokes or flood-fill into mask texture.
