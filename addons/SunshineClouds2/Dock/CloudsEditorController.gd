...
@export var draw_strength: HSlider  ## The compute shader file resource for mask drawing.
...
## Holds the GPU texture RID for the current drawing mask. Needs manual cleanup.
...
## The rendering device used for compute operations.
var rd: RenderingDevice
## Holds the GPU shader RID. Needs manual cleanup.
var shader: RID = RID()
## Holds the GPU compute pipeline RID. Needs manual cleanup.
var pipeline: RID = RID()
## Holds the GPU uniform set RID. Needs manual cleanup.
var uniform_set: RID
	...
	if current_draw_mode != DRAWINGMODE.NONE:
		...
		if drawing_currently:
			RenderingServer.call_on_render_thread(
				execute_compute.bindv([delta, false, Color.WHITE])
			)
...
func initial_scene_load() -> void:
	...
	var file: FileAccess = FileAccess.open(
		"res://addons/SunshineClouds2/CloudsInc.comp", FileAccess.READ_WRITE
	)
...
func initialize_mask_texture() -> void:
	...
	if ResourceLoader.exists(mask_file_path.text):
		...
		if !image || image.get_width() != mask_resolution.value:
			...
			# gdlint: disable=max-line-length
			print(
				"mask incorrect size found size:",
				image.get_width(),
				" desired:",
				mask_resolution.value
			)
			image = Image.create(
				mask_resolution.value, mask_resolution.value, false, Image.FORMAT_RGBAF
			)
			image.clear_mipmaps()
		...
		var image: Variant = Image.create(
			mask_resolution.value, mask_resolution.value, false, Image.FORMAT_RGBAF
		)
		image.clear_mipmaps()
		image.save_exr(mask_file_path.text)
...
func iterate_cursor_location(viewport_camera: Camera3D, event: InputEventMouse) -> void:
	if is_instance_valid(driver) && driver.clouds_resource != null:
		current_clouds_height = (
			(driver.clouds_resource.cloud_floor + driver.clouds_resource.cloud_ceiling) / 2.0
		)
	else:
...
func set_draw_scale() -> void:
	if driver != null && driver.clouds_resource != null:
		draw_brush_tool.scale = Vector3(
			draw_scale,
			driver.clouds_resource.cloud_ceiling - driver.clouds_resource.cloud_floor,
			draw_scale
		)
	else:
...
func enable_draw_mode() -> bool:
	...
	if !is_instance_valid(draw_brush_tool):
		print("CloudsEditorController: enable_draw_mode() called. Spawning draw brush tool.")
...
func disable_draw_mode() -> void:
	...
	if is_instance_valid(draw_brush_tool):
		print("CloudsEditorController: disable_draw_mode() called. Despawning draw brush tool.")
	...
	if last_image_data.size() > 0:
		...
		var image: Image = Image.create_from_data(
			mask_resolution.value, mask_resolution.value, false, Image.FORMAT_RGBAF, last_image_data
		)
		image.save_exr(mask_file_path.text)
